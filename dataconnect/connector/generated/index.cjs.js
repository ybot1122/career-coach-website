...
exports.meetingsByConsultantId = function meetingsByConsultantId(dcOrVars, varsOrOptions, options) {
  
  const { dc: dcInstance, vars: inputVars, options: inputOpts } = validateArgsWithOptions(connectorConfig, dcOrVars, varsOrOptions, options, true, true);
  return executeQuery(meetingsByConsultantIdRef(dcInstance, inputVars), inputOpts && { fetchPolicy: inputOpts.fetchPolicy });
}
;
...
allConsultantProfilePicturesAndIdsRef.operationName = 'AllConsultantProfilePicturesAndIds';
exports.allConsultantProfilePicturesAndIdsRef = allConsultantProfilePicturesAndIdsRef;

exports.allConsultantProfilePicturesAndIds = function allConsultantProfilePicturesAndIds(dcOrOptions, options) {
  
  const { dc: dcInstance, vars: inputVars, options: inputOpts } = validateArgsWithOptions(connectorConfig, dcOrOptions, options, undefined,false, false);
  return executeQuery(allConsultantProfilePicturesAndIdsRef(dcInstance, inputVars), inputOpts && { fetchPolicy: inputOpts.fetchPolicy });
}
;
...
exports.getMeeting = function getMeeting(dcOrVars, varsOrOptions, options) {
  ...
  const { dc: dcInstance, vars: inputVars, options: inputOpts } = validateArgsWithOptions(connectorConfig, dcOrVars, varsOrOptions, options, true, true);
  return executeQuery(getMeetingRef(dcInstance, inputVars), inputOpts && { fetchPolicy: inputOpts.fetchPolicy });
}
;
...
updateMeetingCalendlyScheduleUrlRef.operationName = 'UpdateMeetingCalendlyScheduleUrl';
exports.updateMeetingCalendlyScheduleUrlRef = updateMeetingCalendlyScheduleUrlRef;

exports.updateMeetingCalendlyScheduleUrl = function updateMeetingCalendlyScheduleUrl(dcOrVars, vars) {
  const { dc: dcInstance, vars: inputVars } = validateArgs(connectorConfig, dcOrVars, vars, true);
  return executeMutation(updateMeetingCalendlyScheduleUrlRef(dcInstance, inputVars));
}
;
...
exports.updateUserProfilePicture = function updateUserProfilePicture(dcOrVars, vars) {
  const { dc: dcInstance, vars: inputVars } = validateArgs(connectorConfig, dcOrVars, vars, true);
...
exports.meetingsForCurrentUser = function meetingsForCurrentUser(dcOrOptions, options) {
  
  const { dc: dcInstance, vars: inputVars, options: inputOpts } = validateArgsWithOptions(connectorConfig, dcOrOptions, options, undefined,false, false);
  return executeQuery(meetingsForCurrentUserRef(dcInstance, inputVars), inputOpts && { fetchPolicy: inputOpts.fetchPolicy });
}
;
